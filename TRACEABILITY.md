...
| Paused readiness clock | readiness deadline across `signal stop` and `signal cont` during startup | `pausing_suspends_the_readiness_deadline`; `a_timer_in_a_paused_state_disarms_instead_of_faulting` | `continuing_rearms_the_readiness_deadline`; `continuing_a_ready_generation_arms_no_deadline` |
| Lifecycle wait outcomes | settled `failed`/`exited` supervisors while awaiting start, restart, once, and stop goals | `a_settled_supervisor_abandons_a_start_goal_immediately`; `an_abandoned_lifecycle_is_reported_as_unavailable` | `a_settled_supervisor_satisfies_a_stop_goal`; `a_working_supervisor_keeps_the_client_waiting` |
| `immortalctl` short help | `-h` no longer aliases `hup`; both help spellings display help | `short_help_displays_help_instead_of_signalling`; `routing_contract` short-help route | `hup_remains_available_as_an_explicit_signal`; `all_released_signal_flags_are_normalized` |
| Peer authorization refusal | unauthorized control peer response code and client slot accounting | `listener_tells_an_unauthorized_peer_why_it_was_refused` | `rejected_peers_release_their_client_slot` |
| Schema version binding | `version: 2` enforcement inside the current-version decoder | `parse_current_rejects_a_document_which_is_not_version_two` | `parse_current_accepts_version_two` |
| Control socket bind cleanup | socket inode left behind by a rejected bind | `listener_refuses_unsafe_or_existing_paths` | `listener_cleanup_never_removes_a_replacement` |
| Rotation age and retention races | live-file age across adapter restarts; archives removed by another process mid-sweep | `reopening_does_not_reset_the_age_clock`; `a_new_destination_is_not_immediately_aged_out` | `a_concurrent_remover_never_fails_retention`; `archives_tolerates_a_concurrent_remover`, both scheduling the race against a churning thread |
| Definition limit | surviving definitions when a directory exceeds `max_definitions` | `isolates_invalid_files_and_enforces_definition_limit` | `enforces_the_definition_limit_in_a_stable_order`, proving the kept set is lexicographic and repeatable rather than enumeration-ordered |
| Watcher loss | closed native event channel during and outside a debounce window | `a_lost_watcher_reports_once_and_falls_back_to_periodic_scans`; `a_lost_watcher_stops_being_reported_after_the_first_scan` | `a_lost_watcher_still_lets_the_debounce_window_close`, proving the debounce deadline resolves instead of spinning |
