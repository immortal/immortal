...
/// File adapter failure.
#[derive(Debug)]
pub struct ActionError(io::Error);

impl Display for ActionError {
    fn fmt(&self, formatter: &mut Formatter<'_>) -> fmt::Result {
        write!(formatter, "logging adapter failed: {}", self.0)
    }
}
...
/// Stream stdin to the rotating sink and optional downstream logger.
///
/// # Errors
///
/// Returns an error for destination, input, output, sync, or rotation failure.
pub fn execute(action: &Action) -> Result<(), ActionError> {
    let mut sink = RotatingFile::open(&action.file, action.rotation).map_err(ActionError)?;
    let stdin = io::stdin();
    let mut input = stdin.lock();
    if action.passthrough {
        let stdout = io::stdout();
        let mut output = stdout.lock();
        copy_stream(&mut input, &mut sink, Some(&mut output), action.timestamp).map_err(ActionError)
    } else {
