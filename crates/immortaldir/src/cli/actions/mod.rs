...
use immortal_core::{
    ...
    exit::ExitClass,
    process::ProcessBrokerEndpoint,
    reconcile::{
        DefinitionSnapshots, DependencyError, DesiredStateTracker, LauncherError, ReconcileAction,
        ScanError, ScanLimits, ScanResult, SupervisorLaunch, SupervisorLauncher,
...
impl ActionError {
    ...
    pub const fn exit_class(&self) -> ExitClass {
        match self {
            ...
            Self::Launcher(_) => ExitClass::OsError,
...
mod tests {
    ...
    use super::{
        ActionError, DesiredPlan, ServiceFailure, ServiceFailureKind, desired_plan, execute,
        write_plan,
    };
    use crate::cli::dispatch::Action;
    ...
    async fn one_shot_dry_run_scans_successfully() -> Result<(), Box<dyn Error>> {
        ...
        execute(&action(directory.path(), true), None).await?;
        Ok(())
    }
    ...
    async fn mutation_mode_requires_a_pre_runtime_broker() -> Result<(), Box<dyn Error>> {
        let directory = TestDirectory::new()?;
        assert!(matches!(
            execute(&action(directory.path(), false), None).await,
